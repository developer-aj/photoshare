...
def album(request, pk, view="thumbnails"):
	...
	for img in images.object_list:
		tags = x[x[1] for x in img.albums.values_list()]
...
def image(request, pk):
	...
	return render_to_response("poto/image.html", dict(image=img, user=request.user, backurl=request.META["HTTP_REFERER"], media_url=MEDIA_URL))

def update(request):
	"""Update image title, rating, tags, albums."""
	p = request.POST
	images = default.dict(dict)
